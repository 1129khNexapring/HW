									...
									<form action="/chat/adminWrite.kh" method="post">
										<div class="main-chat">
											...
											<c:forEach items="${room}" var="chatroom1" begin="0" end="0">
												<input type="hidden" name="recv_mem"
													value="${chatroom1.memberId}" />
												<div class="mId">${chatroom1.memberId}</div>
											</c:forEach>

											<input type="hidden" name="name"
												value="${loginAdmin.memberName}" /> <input type="hidden"
												name="phone" value="${loginAdmin.memberPhone}" /> <input
												type="hidden" name="memberId" value="${loginAdmin.memberId}" />
												...
												<div class="chat-content">
													<c:if test="${chatroom1.name ne 'admin' }">
														<div class="me-chat">
															...
															<span class="time"><fmt:parseDate var="daVal3"
																	value="${chatroom1.reg_date}"
																	pattern="yyyy-MM-dd HH:mm:ss" /> <fmt:formatDate
																	value="${daVal3}" type="time" /></span>

														</div>
													</c:if>
													<c:if
														test="${chatroom1.memberId=='admin' && param.memberId==chatroom1.recv_mem}">
											...
											<div class="insert-adcontent">
												<textarea id="chat-insert" name="content" class="textarea"
